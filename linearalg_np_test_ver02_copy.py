...
def cal_linear_alg_lstsq(df_landcover, df_lst, df_emiswb, df_emis_repre, row, col, kernel_l,moving_pixel=4):
    ...
    for nrow in range(row):
        for ncol in range(col):        
            ...
            if fraction_map[:,nrow,ncol].mean() == -9999 or indepent_matrix[nrow,ncol].mean() == -9999 or emis_matrix[nrow,ncol].mean() == -9999:
                pass
            else:
    ...
    # Try small row and col for testing 
    #row = 150
    #col = 150
...
def least_sq(coeff_df, indep_df, radiance, bounds, _type):
    ...
    if radiance is True:
        ...
        coeff_df['residual'] = np.concatenate([[res.fun[i]]*8 for i in range(len(res.fun))])
...
#    import os
#    import pickle
#    path = '/nas/rhome/tvo/py_code/aes509/data/wholy_nyc_70m_modified/'
#    for folder in os.listdir(path):
#        
#        if folder != 'df_landcover_concat':
#            date_code = folder
#            f = open(path + folder + '/' + 'bounds.pckl','rb')
#            bound_min, bound_max = pickle.load(f)
#            f.close()
#            print(folder)
#            print(bound_min)
#            
#            main(path, date_code, bound_min, bound_max)
#                
    ...
    '''
    New version: 05.17.2020- Running algrorithm for multiple days
    '''
    path = '/nas/rhome/tvo/py_code/aes509/data/wholy_nyc_70m_modified_copy/'
    for folder in os.listdir(path):
        try:
            #if folder != 'df_landcover_concat' and os.path.isdir(path + folder + '/out_value_list_kernel_10_moving_5/') is False:
            if folder == '2019-07-04T03-34-56': # Try for a specfific date
                ...
                path_sub = path+date_code +'/'# certain date 
            
                
                # Step 0: Load required files
                df_landcover_concat = pd.read_pickle(path+'df_landcover_concat')
                df_emiswb = pd.read_csv(path_sub+'df_emiswb')
                df_lst = pd.read_csv(path_sub+'df_lst')
                df_emis_repre = pd.read_csv(path_sub+'df_emis_repre')
                ...
                # Make a directory for different bounds values
                try:
                    os.mkdir(path_sub + str(bound_min)+'_'+str(bound_max))
                except:
                    pass
                
                path_sub = path_sub + str(bound_min)+'_'+str(bound_max)+'/'
...
#                    
                ...
                #print(results[0])
                # Export to pickle for each date
    #            try:
    #                results[0][0].to_pickle(path_sub + 'coeff_df_matrix')
    #                results[0][1].to_pickle(path_sub + 'indep_df_matrix')
    #            except Exception as e:
    #                print(e)
    #                pass
                ...
                for i in range(len(results_total)):
                    ...
                    try:
                        ...
                        for _,frame in enumerate(out_value_list_split):
                            try:
                                os.mkdir(path_sub+'out_value_list_kernel_'+str(kernel_list[i][0])+'_moving_'+str(moving_pixel[i])+'/')
                            except Exception as e:
                                print(e)
                                pass
                            
                            frame.to_csv(path_sub+'out_value_list_kernel_'+str(kernel_list[i][0])+'_moving_'+str(moving_pixel[i])+'/out_value_list_'+str(_) + '.csv', index=False)
                    ...
                    # In case encounter memory issue, split the results list to multi lists 
                        ...
                        for k in range(len(results)):
                            try:
                                os.mkdir(path_sub+'out_value_list_kernel_'+str(kernel_list[i][0])+'_moving_'+
                                         str(moving_pixel[i])+'/')
                            except Exception as e:
                                print(e)
                                pass
                            
                            results[k].to_csv(path_sub+'out_value_list_kernel_'+str(kernel_list[i][0])+'_moving_'+
